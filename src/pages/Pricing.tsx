...
function PlanCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.03] p-6 flex flex-col transition-colors">
      {/* Name & description */}
      ...
      <div className="mt-5">
        {plan.monthlyPrice !== null ? (
          ...
          <>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Flexible plans</p>
          </>
        )}
      </div>
        ...
        <ul className="space-y-2.5">
          {plan.features.map((f) => (
            <li key={f} className="flex items-start gap-2.5 text-sm text-gray-600 dark:text-gray-300">
              <Check size={15} className="mt-0.5 shrink-0 text-gray-400 dark:text-gray-500" />
              {f}
            </li>
          ))}
        </ul>
      ...
      <div>
        ...
        <div className="flex items-center gap-1.5 mt-3 sm:hidden">
          {['ISO 27001', 'GDPR', 'SOC 2'].map((badge) => (
            <span
              key={badge}
              className="px-2.5 py-1 text-[10px] font-bold bg-blue-100 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-200 dark:border-blue-500/30"
            >
              {badge}
            </span>
          ))}
      ...
      <div className="hidden sm:flex items-center gap-2">
        <div className="flex gap-1.5">
          {['ISO 27001', 'GDPR', 'SOC 2'].map((badge) => (
            <span
              key={badge}
              className="px-3 py-1.5 text-[11px] font-bold bg-blue-100 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-200 dark:border-blue-500/30"
            >
              {badge}
            </span>
          ))}
