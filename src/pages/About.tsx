      ...
      <main className="min-h-screen bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white font-['Inter',sans-serif] overflow-hidden transition-colors duration-300">
        ...
        <header className="border-b border-gray-200 dark:border-white/10 px-6 md:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <Link to="/credits" className="text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors hidden sm:inline-block">Credits</Link>
        ...
        >
          ...
          <motion.p variants={fadeUp} custom={1} className="mt-4 text-gray-500 dark:text-gray-400 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">
          ...
          <motion.p variants={fadeUp} custom={2} className="mt-6 text-gray-400 dark:text-gray-500 text-sm max-w-2xl mx-auto leading-relaxed">
        ...
        <section className="max-w-5xl mx-auto px-6 pb-16">
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={staggerContainer}>
            {WHY_CARDS.map((c, i) => (
              <motion.div key={c.title} variants={fadeUp} custom={i} whileHover={{ y: -4, transition: { duration: 0.2 } }} className="rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.03] p-6 transition-colors">
                <c.icon size={28} className="text-blue-500 dark:text-blue-400 mb-4" />
            ...
            {WHO_CARDS.map((c, i) => (
              <motion.div key={c.title} variants={fadeUp} custom={i} whileHover={{ y: -4, transition: { duration: 0.2 } }} className="rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.03] p-6 transition-colors">
                ...
                <ul className="space-y-2">
                  {c.items.map((item) => (
                    <li key={item} className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed flex items-start gap-2">
        ...
        <section className="max-w-5xl mx-auto px-6 pb-16">
          ...
          <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={staggerContainer}>
            {BUILD_ITEMS.map((b, i) => (
              <motion.div key={b.label} variants={fadeUp} custom={i} whileHover={{ scale: 1.03, transition: { duration: 0.2 } }} className="rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.03] p-5 flex items-center gap-4 transition-colors">
                <b.icon size={22} className="text-blue-500 dark:text-blue-400 shrink-0" />
                <span className="text-sm font-semibold text-gray-600 dark:text-gray-300">{b.label}</span>
              </motion.div>
            ))}
          </motion.div>
        </section>
        ...
        <section className="max-w-4xl mx-auto px-6 pb-16">
          ...
          <div className="space-y-5">
            {WORKFLOW_STEPS.map((s, i) => (
              <motion.div key={s.step} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ delay: i * 0.12, duration: 0.5, ease: "easeOut" }} className="rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.03] p-6 flex items-start gap-5 transition-colors">
                <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-600/20 flex items-center justify-center shrink-0">
                  <span className="text-blue-600 dark:text-blue-400 font-black text-lg">{s.step}</span>
                </div>
                <div>
        ...
        <motion.section className="max-w-4xl mx-auto px-6 pb-16" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={staggerContainer}>
          ...
          <motion.p variants={fadeUp} custom={1} className="text-center text-gray-500 dark:text-gray-400 text-sm max-w-xl mx-auto mb-8 leading-relaxed">
          ...
          <motion.div variants={fadeUp} custom={2} className="flex justify-center gap-3 mb-8">
            {["ISO 27001", "GDPR", "SOC 2"].map((badge) => (
              <span key={badge} className="px-4 py-2 text-xs font-bold bg-blue-100 dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-200 dark:border-blue-500/30">
                {badge}
              </span>
            ))}
          </motion.div>
          <motion.p variants={fadeUp} custom={3} className="text-center text-gray-400 dark:text-gray-500 text-sm max-w-lg mx-auto leading-relaxed">
